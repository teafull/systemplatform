...
app.get('/health', (req, res) => {
  res.json({
    ...
    service: 'authorization-center'
  });
});

// API路由
app.use('/api/authz', authorizationRoutes);
...
async function startServer() {
  try {
    // 初始化数据库
    await initDatabase();
    console.log('Authorization center database initialized');
    ...
    // 启动HTTP服务器
    app.listen(PORT, () => {
      console.log(`Authorization Center server running on port ${PORT}`);
